...
class vanilla_option_mc:  public monte_carlo_pricer{
private :
    double expiry;
    double strike;
    double spot;
...
public :
    //  Default constructor
    vanilla_option_mc(): expiry(0.0) ,strike(0.0), spot(0.0) , vol(0.0), r(0.0), numPaths(0) , a('c') {}

    // Main constructor
    vanilla_option_mc(double expiry, double strike, double spot, double vol, double r, unsigned long numPaths, char a)
...
double pricer() const override
{
    ...
    for (unsigned long i = 0; i < numPaths; ++i)
    {
        ...
        double this_payoff = (a == 'c') ? (this_spot - strike) : (strike - this_spot);
        if (this_payoff > 0.0) running_sum += this_payoff;
    }
